...
if [ -e "resource_ids" ]; then
    ...
    STATE=$(aws ec2 describe-instances --query "Reservations[].Instances[?InstanceId=='$I_ID'].State.Name" --output text --profile admin)
    ...
    if [ $STATE = 'running' ]; then
        IP=$(aws ec2 describe-instances --query "Reservations[].Instances[?InstanceId=='$I_ID'].PublicIpAddress" --output text --profile admin)
        USER=ubuntu
